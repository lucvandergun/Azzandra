...
namespace Azzandra
{
    public abstract class SpellEffectAcute : ISpellEffect
    {
        protected int GetSpc(Entity attacker)
        {
            if (attacker == null)
                return 1;
        ...
        public SpellEffectAcute() { }

        public virtual int GetStrength(int level) => 1;

        public abstract void Apply(Entity attacker, int level);
