...
namespace Azzandra
{
    public abstract class Visibility
    {
        ...
        /// </param>
        public abstract void Compute(Vector origin, int rangeLimit);
    }
    ...
    public sealed class MyVisibility : Visibility
    {
        ...
        public override void Compute(Vector origin, int rangeLimit)
        {
            ...
            for (int octant = 0; octant < 8; octant++)
            {
                ...
                ComputeOctant(origin, octant, rangeLimit);
            }
        }
        ...
        ///   5 6
        /// </summary>
        public void ComputeOctant(Vector origin, int octant, int rangeLimit)
        {
            Compute(octant, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
        }
        ...
        struct Slope // represents the slope Y/X as a rational number
        {
            ...
            public bool Less(int y, int x) { return Y * x < X * y; } // this < y/x
        ...
        void Compute(int octant, Vector origin, int rangeLimit, int x, Slope top, Slope bottom)
        {
            ...
            // Automatic range limit of 100 if supplied value is less than zero.
            for (; (rangeLimit < 0 && x <= 100) || x <= rangeLimit; x++) // (x <= (int)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
            {
        ...
        bool BlocksLight(int x, int y, int octant, Vector origin)
        {
        ...
        void SetVisible(int x, int y, int octant, Vector origin)
        {
            int nx = origin.X, ny = origin.Y;
            switch (octant)
            {
