...
namespace Azzandra.Generation
{
    public class SpawnData
    {
        public static Dictionary<Type, SpawnData> DataDictionary = new Dictionary<Type, SpawnData>()
        {
            ...
            { typeof(Skeleton),     new SpawnData(2, 3, 4, 1, 14, Temp.Glacial, Temp.Scorching) },
            { typeof(ArmouredSkeleton),new SpawnData(2, 3, 3, 15, 29, Temp.Glacial, Temp.Scorching) },
            { typeof(Wraith),       new SpawnData(5, 1, 1, 6, 29, Temp.Glacial, Temp.Scorching) },
            { typeof(RockGolem),    new SpawnData(4, 1, 1, 5, 15, Temp.Cold, Temp.Scorching) },
            { typeof(CrystalGolem), new SpawnData(3, 1, 1, 19, 29, Temp.Cold, Temp.Scorching) },

            { typeof(IceElemental), new SpawnData(2, 1, 2, 3, 9, Temp.Glacial, Temp.Cold) },
            { typeof(IceGiant),     new SpawnData(15, 1, 2, 6, 11, Temp.Glacial, Temp.Cold) },
            { typeof(Wolf),         new SpawnData(1, 4, 5, 1, 7, Temp.Freezing, Temp.Lukewarm) },
            { typeof(Ghast),        new SpawnData(5, 1, 2, 5, 10, Temp.Cold, Temp.Hot) },
            ...
            { typeof(Ghaugh),       new SpawnData(3, 2, 3, 8, 13, Temp.Glacial, Temp.Cold) },
            { typeof(CaveWorm),    new SpawnData(5, 1, 2, 1, 7, Temp.Cold, Temp.Hot) },
            { typeof(LostMage),     new SpawnData(5, 1, 1, 1, 6, Temp.Cold, Temp.Hot) },

            { typeof(VampireBat),   new SpawnData(3, 1, 3, 16, 22, Temp.Lukewarm, Temp.Hot) },
            { typeof(GelatinousCube),new SpawnData(8, 1, 2, 10, 16, Temp.Cold, Temp.Warm) },
            { typeof(Specter),      new SpawnData(10, 1, 1, 13, 21, Temp.Lukewarm, Temp.Warm) },
            { typeof(VineBlight),   new SpawnData(8, 1, 2, 11, 17, Temp.Lukewarm, Temp.Warm) },
            { typeof(ToxicSpider),  new SpawnData(3, 3, 3, 12, 19, Temp.Lukewarm, Temp.Warm) },
            { typeof(WolfSpider),   new SpawnData(6, 1, 1, 15, 21, Temp.Lukewarm, Temp.Warm) },
            { typeof(CarnivorousPlant),new SpawnData(8, 1, 2, 13, 19, Temp.Lukewarm, Temp.Hot) },
            { typeof(Troll),        new SpawnData(10, 1, 2, 9, 15, Temp.Freezing, Temp.Hot) },
            { typeof(Necromancer),  new SpawnData(5, 1, 1, 8, 19, Temp.Freezing, Temp.Hot) },
            { typeof(ShadowHound),  new SpawnData(4, 2, 3, 14, 21, Temp.Cold, Temp.Hot) },
            { typeof(Ooze),         new SpawnData(1, 1, 3, 17, 22, Temp.Lukewarm, Temp.Hot) },

            { typeof(Demon),        new SpawnData(12, 1, 1, 25, 29, Temp.Hot, Temp.Scorching) },
            { typeof(FireElemental),new SpawnData(2, 1, 2, 23, 29, Temp.Hot, Temp.Scorching) },
            { typeof(FireGiant),    new SpawnData(10, 1, 2, 24, 29, Temp.Hot, Temp.Scorching) },
            { typeof(Hellhound),    new SpawnData(4, 2, 3, 22, 29, Temp.Hot, Temp.Scorching) },
            { typeof(Cockatrice),   new SpawnData(3, 1, 2, 21, 29, Temp.Lukewarm, Temp.Hot) },
            { typeof(Choker),       new SpawnData(12, 1, 2, 23, 29, Temp.Lukewarm, Temp.Scorching) },
            { typeof(Enchanter),    new SpawnData(6, 1, 1, 20, 29, Temp.Cold, Temp.Scorching) },
            { typeof(DustMephit),   new SpawnData(6, 1, 2, 18, 27, Temp.Warm, Temp.Scorching) },
        };
        ...
        public static readonly List<Tuple<int, Type>> LivingPopulation = new List<Tuple<int, Type>>
        {
            ...
            Tuple.Create(3, typeof(DustMephit)),
        };

        public static readonly List<Tuple<int, Type>> HauntedPopulation = new List<Tuple<int, Type>>
        {
            //Tuple.Create(1, typeof(Wraith)),
            //Tuple.Create(1, typeof(Specter)),
            //Tuple.Create(2, typeof(ShadowElemental)),
            //Tuple.Create(1, typeof(Ghast)),
        };
    }
}
